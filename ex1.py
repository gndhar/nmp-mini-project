...
def run_physics_chunk(seed):
    ...
    for mB, res, mask in [(M_MU, res_mu, is_mu), (M_E, res_e, ~is_mu)]:
        ...
        cc = cos_cm[mask]
        sc = sin_cm[mask]
        tc = theta_cm_deg[mask]

        pCM = (pm**2 - mB**2) / (2.0 * pm)
        EB_cm = np.sqrt(mB**2 + pCM**2)

        pB_par_lab = gam * (pCM * cc) + b * EB_cm
        pC_par_lab = gam * (-pCM * cc) + b * pCM
        p_per_lab = pCM * sc
        ...
        res[0] = fast_binning(pp, MOM_BINS)
        res[1] = fast_binning(tc, ANG_BINS)
