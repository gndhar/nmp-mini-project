...
# Binning Definitions (Low, High, N_Bins)
# Using tuples for Numba compatibility
MOM_BINS = (0.0, 500.0, 100)
ANG_BINS = (0.0, 180.0, 90)
PROF_BINS = (0.0, 500.0, 25)

# ---------------------------------------------------------------------------
# Numba JIT Optimized Physics + Binning Kernel
# ---------------------------------------------------------------------------


@njit(fastmath=True, cache=True)
def fast_binning(data, bin_params):
    """Manual digitize + bincount for uniform bins."""
...
def run_physics_chunk(seed):
    """
    Combined kernel:
    1. Generates parent pion.
    2. Decays it into muon AND electron branches.
    3. Performs Lorentz boosts and angular math.
    4. Bins everything into integer arrays before returning.
    """
    np.random.seed(seed)
    n = CHUNK_SIZE

    # 1. Generate Parent Pion (Shared for both species)
    pi_mass = np.random.normal(M_PI, SIG_PI, n)
    pi_mom = np.random.exponential(EXP_MOM_MEAN, n)

    # Pre-calculate Lorentz factors
    E_pi = np.sqrt(pi_mass**2 + pi_mom**2)
    gamma = E_pi / pi_mass
    bg = pi_mom / pi_mass

    # CM decay angles (Isotropic)
    cos_cm = np.random.uniform(-1.0, 1.0, n)
    sin_cm = np.sqrt(1.0 - cos_cm**2)
    theta_cm_deg = np.arccos(cos_cm) * (180.0 / np.pi)

    # Storage for results [Species][Histogram_Index]
    # Indices: 0: pPi, 1: tB_cm, 2: pB_lab, 3: pC_lab, 4: tB_lab, 5: tC_lab, 6: open, 7: pn, 8: psy, 9: psy2
    ...
    for mB, res in [(M_MU, res_mu), (M_E, res_e)]:
        # Physics: Neutrino is massless, so pCM is simplified:
        # pCM = (m_pi^2 - m_B^2) / (2 * m_pi)
        pCM = (pi_mass**2 - mB**2) / (2.0 * pi_mass)
        EB_cm = np.sqrt(mB**2 + pCM**2)

        # LAB transformation (Lorentz Boost)
        pB_par_lab = gamma * (pCM * cos_cm) + bg * EB_cm
        pC_par_lab = gamma * (-pCM * cos_cm) + bg * pCM  # Nu energy in CM is pCM
        ...
        # Profile Data (Opening Angle vs Pi Momentum)
...
if __name__ == "__main__":
    # MacBook specific: Use 'spawn' for safe multiprocessing with ROOT
    try:
    ...
    print(f"Executing {N_CHUNKS} chunks of {CHUNK_SIZE} events...")
    with multiprocessing.Pool(processes=N_WORKERS) as pool:
        # imap_unordered is slightly faster as it yields results as they finish
